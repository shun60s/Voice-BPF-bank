...
#   add self.f_min 
#
#   main add argument length, f_min 
        ...
        if LA_ranges is not None:
            ...
            if path0 is not None:  # tube_stack.npz
                np.savez(path0, pks=self.peaks_detail_stack, dpks=self.drop_peaks_detail_stack,\
                 iter=self.iters_stack, misc=[self.MAX_tube_length, self.f_min] )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--length', '-l', type=float, default=26.0, help='specify maximum whole tube length[cm]')
    parser.add_argument('--fmin', '-m', type=float, default=200, help='specify detect minimum frequency [Hz] ')
    ...
    if args.tube == 2:  # try two tube model
        ...
        MAX_tube_length= args.length  #26.   # specify maximum whole tube length
    ...
    # instance
    tube= compute_tube_peak(NUM_TUBE=NUM_TUBE, sampling_rate=sampling_rate, f_min=args.fmin)  #, disp=True)
    
    path0= 'pks_dpks_stack_tube' + str(tube.NUM_TUBE) + '.npz'
    # new pre-compute
    print ('MAX_tube_length, f_min ', MAX_tube_length, tube.f_min)
