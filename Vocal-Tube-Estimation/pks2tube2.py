...
from get_fp6 import *
from tube_peak2 import *
from pre_compute2 import *
...
def show_figure2(tube, tube2, spec0, fout_index, fout_index2, df0, fmin0, LA0, path0=None):
    # This shows from f_min=200(fixed)
    
    # comparison frequency response of tube with wav
    index_f_min= int( np.ceil(tube2.f_min / df0) )
    index_f_max= int( np.trunc(tube.f_max / df0) + 1) 
    f0= np.arange( index_f_min * df0, index_f_max * df0, df0 )
    resp0= spec0[index_f_min : index_f_max]
    ...
    plt.ylabel('Amplitude [dB]')
    # tube spectrum
    ax1.semilogy(tube2.f, tube2.response, 'b', ms=2)
    ax1.semilogy(tube.f[tube.peaks_list] , tube.response[tube.peaks_list], 'ro', ms=3)
    ax1.semilogy(tube.f[tube.drop_peaks_list] , tube.response[tube.drop_peaks_list], 'co', ms=3)
    # wav lpc spectrum
    ax1.semilogy( f0 , resp0, 'g', ms=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--wav_file', '-w', default='../wav/i_1-16k_hpf.wav', help='specify input wav-file-name(mono,16bit,16Khz)')
    ...
    # set f_min to match pre-computed grid data
    tube.set_f_min( pc1.f_min)
    # show MAX_tube_length and f_min
    print ('MAX_tube_length, f_min ', pc1.MAX_tube_length, pc1.f_min)
    
    # instance
    fp5= Class_get_fp(f_min= pc1.f_min)
    ...
    for l, nfame in enumerate( frame_list):
        ...
        # one more instance f_min=200
        tube2= compute_tube_peak(NUM_TUBE=NUM_TUBE, sampling_rate=sampling_rate)  #, disp=True)
        tube2(res_brute[0])
        
        show_figure2(tube, tube2, spec_out[l], fout_index[l], fout_index2[l], fp5.df0, res_brute[1], res_brute[0], path0=path0)
